...
def apply(conf_file: str, dry_run: bool, batch_size: int, delay: int) -> None:
    ...
    for resource in resources:
        ...
        if not tags:
            ...
            logger.info(
                f"Adding resource {resource.get_arn()} to a batch with tags {tags}"
            )
...
def _process_batch(dry_run: bool, delay: int, logger, client, batch: dict) -> None:
    if dry_run is False:
        logger.info(
            f'Applying {len(batch["resource_arns"])} resource(s) '
            f'with tags {batch["tags"]}'
        )
        response = client.tag_resources(
            ResourceARNList=batch["resource_arns"], Tags=batch["tags"]
        )
        if (
            "FailedResourcesMap" in response
            and len(response["FailedResourcesMap"].keys()) > 0
        ):
            logger.error(
                "Failed to apply tags to "
