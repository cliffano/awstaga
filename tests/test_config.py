...
class TestConfig(unittest.TestCase):
    ...
    def test_load_with_properties(
        self, func_init, func_open
        ...
        mock_logger.info.assert_has_calls(
            [call("Loading 1 tagset(s)..."), call("Loading 1 resource(s)...")]
        )
        ...
        mock_logger.debug.assert_has_calls(
            [
                call(
                    "Loaded tagset test-tagset with tags "
                    "(test-key1=test-value1, test-key2=test-value2)"
                ),
                call(
                    "Loaded resource test-resource-arn with tags "
    ...
    @patch("awstaga.config.init")
    def test_load_with_properties_having_resource_without_tags(
        self, func_init, func_open
    ):  # pylint: disable=unused-argument
        with open("awstaga.yaml", "r", encoding="utf8") as file_handle:
            assert (
        ...
        mock_logger.debug.assert_has_calls(
            [
                ...
                call(
                    "Loaded resource test-resource-arn with tagsetnames ('test-tagset',)"
                ),
            ]
        )
    ...
    @patch("awstaga.config.init")
    def test_load_with_properties_having_resource_without_tagsetnames(
        self, func_init, func_open
    ):  # pylint: disable=unused-argument
        with open("awstaga.yaml", "r", encoding="utf8") as file_handle:
            assert (
    ...
    @patch("awstaga.config.init")
    def test_load_with_properties_without_tagsets(
        self, func_init, func_open
    ):  # pylint: disable=unused-argument
        with open("awstaga.yaml", "r", encoding="utf8") as file_handle:
        ...
        mock_logger.info.assert_has_calls([call("Loading 1 resource(s)...")])
    ...
    @patch("awstaga.config.init")
    def test_load_with_properties_having_includes(
        self, func_init, func_open
    ):  # pylint: disable=unused-argument
